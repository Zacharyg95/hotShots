...
public class HotShotsSeachEngine implements SearchEngine {
   ...
   public List<IndexedFile> getIndexedFiles(){
      ...
      try {
         ...
         System.err.println("Apparently, the search index file does not exist: " + e.getMessage());
      } catch (IOException e) {
         System.err.println("Failed to read lines from our search index file: " + e.getMessage());
         e.printStackTrace();
      }finally{
         if(reader != null){
            try {
               ...
               //swallow the exception because there's nothing we can do about it anyways
            }
         }
      }
   ...
   /**
    * returns an IndexedFile, if the file exists, and the file is a .txt file. Returns null otherwise
    * @param file
    * @return
    */
   private IndexedFile addFileToSearchIndex(File file) {
      ...
      try {
         ...
         writer.write(String.format("%s%s%s%d", NEWLINE, file.getAbsolutePath(), SEARCH_INDEX_FILE_DELIMITER, now.getTime()));
      } catch (IOException e) {
         System.err.println("Failed to add file to our search index: " + e.getMessage());
         e.printStackTrace();
      }finally{
         if(writer != null){
            try {
